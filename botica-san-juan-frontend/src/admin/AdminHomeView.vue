  ...
  <div class="p-4 sm:p-6 lg:p-8 space-y-6">
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
      <div class="rounded-2xl bg-botica-700 p-6 text-white shadow-xl">
        ...
        <p class="mt-1 text-sm text-botica-50">
          Activos
        </p>
      </div>
      <div class="rounded-2xl bg-botica-700 p-6 text-white shadow-xl">
        ...
        <p class="mt-1 text-sm text-exito-50">
          Registrados
        </p>
      </div>
      <div class="rounded-2xl bg-botica-700 p-6 text-white shadow-xl">
      ...
      <div class="rounded-2xl bg-alerta-700 p-6 text-white shadow-xl">
        ...
        <p class="mt-1 text-sm text-alerta-50">
          Acumuladas
        </p>
      </div>
    </div>
      ...
      <div class="rounded-2xl bg-superficie-elevada p-6 shadow-lg ring-1 ring-borde-sutil/80">
        ...
        >
          <div
            ...
            :key="item.nombre"
            class="rounded-2xl bg-superficie-hundida px-4 py-3"
          >
            <div class="flex items-center justify-between gap-4">
              <div>
                <p class="font-semibold text-texto-primario">
              ...
              <p class="font-semibold text-exito-600">
    ...
    <div class="rounded-2xl bg-superficie-elevada p-6 shadow-lg ring-1 ring-borde-sutil/80">
      ...
      <div class="mt-4 grid grid-cols-2 gap-4 md:grid-cols-4">
        <button
          class="rounded-2xl bg-botica-50 p-4 text-left transition hover:bg-botica-50"
          @click="$router.push('/admin/products/add')"
        >
          <Package class="h-7 w-7 text-texto-marca" />
          <p class="mt-3 text-sm font-semibold text-texto-primario">
            Nuevo producto
          </p>
        </button>
        <button
          class="rounded-2xl bg-botica-50 p-4 text-left transition hover:bg-botica-100"
          @click="$router.push('/admin/clients')"
        >
          <Users class="h-7 w-7 text-botica-600" />
          <p class="mt-3 text-sm font-semibold text-texto-primario">
            Ver clientes
          </p>
        </button>
        <button
          class="rounded-2xl bg-exito-50 p-4 text-left transition hover:bg-exito-50"
          @click="$router.push('/admin/orders')"
        >
          <ShoppingCart class="h-7 w-7 text-exito-600" />
          <p class="mt-3 text-sm font-semibold text-texto-primario">
            Ver pedidos
          </p>
        </button>
        <button
          class="rounded-2xl bg-alerta-50 p-4 text-left transition hover:bg-alerta-50"
          @click="$router.push('/admin/reports')"
        >
          <BarChart3 class="h-7 w-7 text-alerta-600" />
          <p class="mt-3 text-sm font-semibold text-texto-primario">
            Reportes
          </p>
        </button>
...
const statusBadge = (status: string) => {
  switch (status) {
    ...
    case 'procesando':
      return 'bg-botica-50 text-botica-800'
    case 'enviado':
      return 'bg-alerta-50 text-alerta-700'
    case 'entregado':
      return 'bg-exito-50 text-exito-700'
    default:
      return 'bg-superficie-interactiva text-texto-primario'
  }
}
